...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2.5 sm:px-6 lg:px-8 py-2 sm:py-8 space-y-2.5 sm:space-y-8">
      {/* Hero Header Section - Ultra Compact Mobile Layout */}
      <section className="bg-gradient-to-r from-emerald-600 to-teal-700 text-white rounded-xl sm:rounded-3xl p-3 sm:p-8 shadow-md space-y-2.5 sm:space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
          <div className="space-y-1 max-w-2xl">
            <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-md text-[8px] sm:text-xs font-bold uppercase tracking-wider">
              <ShieldCheck className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Certified Telemedicine Network
            </div>
            <h1 className="text-sm sm:text-4xl font-extrabold tracking-tight leading-tight">
              Connect With Top Specialists &amp; Book Consultations
            </h1>
            <p className="hidden sm:block text-xs sm:text-sm text-emerald-100 leading-snug">
              Instant video consultations, e-prescriptions, and health vitals tracking.
            </p>
          </div>

          {/* Action Buttons - Side-by-side on mobile, stacked-flex on desktop */}
          <div className="grid grid-cols-2 gap-2 sm:flex sm:gap-2.5 flex-shrink-0">
            <Link
              to="/doctors"
              className="px-2.5 py-1.5 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl bg-white text-emerald-700 font-extrabold text-[10px] sm:text-xs uppercase flex items-center justify-center space-x-1 sm:space-x-1.5 shadow-sm hover:bg-emerald-50 active:scale-95 transition-all text-center"
            >
            ...
            <Link
              to="/book-appointment"
              className="px-2.5 py-1.5 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl bg-amber-400 hover:bg-amber-300 text-slate-950 font-extrabold text-[10px] sm:text-xs uppercase flex items-center justify-center space-x-1 sm:space-x-1.5 shadow-sm active:scale-95 transition-all text-center"
            >
        ...
        <div className="grid grid-cols-5 gap-1 pt-1.5 sm:hidden border-t border-white/20">
          {quickActions.map((act) => {
            ...
            return (
              <Link
                key={act.label}
                to={act.path}
                className="flex flex-col items-center justify-center p-1 rounded-lg bg-white/10 hover:bg-white/20 backdrop-blur-md text-center space-y-0.5 active:scale-95 transition-all"
              >
                <div className={`w-7 h-7 rounded-md ${act.color} flex items-center justify-center shadow-xs`}>
                  <IconComp className="w-3.5 h-3.5" />
                </div>
                <span className="text-[8px] font-bold text-white leading-none tracking-tighter truncate w-full">
                  {act.label.split(' ')[0]}
                </span>
              </Link>
            );
